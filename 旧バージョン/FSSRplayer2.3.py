...
def SRprocess(frames, SRframes, limit = None):
    SRnum = 0
    if len(frames) != len(SRframes):
        del SRframes[len(frames):]

    if method == "FSSR":
        SRnum = SRmethods.FSSR(frames, SRframes, algonum, ign, fps, limit)
        print(SRnum)
    elif method == "NSSR":
...
def SR():
    ...
    SRprocess(frames1, Methods.SRframes1, limit=buftime)
    Methods.finflag = 0
    event.set()
    while True:
...
if __name__ == '__main__':
    ...
    url = "http://192.168.1.142/BBB24fps_144p.mp4"
    ...
    savepath = "{}_{}.mp4".format(title,method)
