...
class Methods:
    ...
    def __init__(self):
        ...
        self.ranking_device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        print(self.device)
        ...
        for n, p in torch.load("./weights/fsrcnn_x4.pth", map_location=lambda storage, loc: storage).items():
            if n in state_dict.keys():
                ...
                raise KeyError(n)
        self.model.eval()
        self.ranking = Ranking(device=self.ranking_device).to(self.ranking_device)
        self.ranking.load_state_dict(torch.load("weights/ranking.pth", map_location=self.ranking_device))
        self.ranking.eval()
    ...
    ##########  Acceleration  #############
    def AFSSR(self, frames, SRframes, ign = 10, fps = 60., limit = None, faststart = False, F = False):
        padding = 7
        m = 0
        block_size = Methods.block_size
        scale = Methods.scale
        ...
        t = torch.Tensor(t).transpose(2,3).transpose(1,2).to(self.ranking_device)
        rank = self.ranking(t).squeeze(1)
